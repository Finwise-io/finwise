...
- **Dates:** `Jun 14` (short), `June 2027` (month‑year goals), relative where friendly ("in 5 days",
  "last month").
- **Numeric input (added 2026‑06‑26, L‑9):** for free‑form number fields, **keep the raw typed text in
  state and parse only on save.** Never run the value through a number parser on every keystroke and write
  it back — typing `10.` becomes `10` and the user can never enter a decimal. Format *displayed* counts to a
  sane precision so float drift never shows (`965.0000001` → `965`).
- **Hide balances (added 2026‑06‑26, B‑74):** masking is applied **once, at the shared formatter**
  (`money`/`moneyCompact` return `••••` when the flag is set), not bolted onto each component — so toggling
  it hides *every* figure app‑wide at once. A screen that hand‑formats a number would leak through the mask;
  another reason to always use the shared formatter.
...
"diversification."
**Implementation (one source of truth):** every definition lives in `src/domain/glossary.ts` (`GLOSSARY`)
and is surfaced via `<InfoDot term="…">` — so a term reads identically everywhere. Coverage now includes the
**asset classes** (Cash, Stocks & ETFs, Bonds, Alternatives, Real estate, Personal property), **Gross
income**, and **Contribution room**; dots are attached to the Net Worth *By‑class* group headers and the
Bonds / Alternatives / Contribution‑room titles. **Adding a term:** add it to `GLOSSARY`, then `<InfoDot>`
it — never write a definition inline on a screen.
...
| **Navigation** | Standard patterns only — **bottom tabs** + native sheets. Minimize the learning curve. **Every new top-level screen must be added to the route-guard allow-list (`MODAL_SEGMENTS`)** or the guard bounces it back to Home = an apparent loop (L‑10). A filesystem test enforces this. |
...
| **Accessibility ratchet** | Every interactive control needs `accessibilityRole`/`accessibilityLabel`. A per-file baseline (`a11y-baseline.json`) can only go **down**. Note: the guard scans the opening tag up to the first `>`, so put a11y props **before** any `onPress={() => …}` arrow or they won't be counted. |
| **Provenance** | A money figure the app derives should be **traceable** — offer a drill-down to the accounts + math behind it (e.g. Insights cards), never an unexplained number (Part 4.2). |
