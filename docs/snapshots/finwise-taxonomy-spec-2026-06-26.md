...
### 2a. Disposing of a holding — *sell* vs *re-mark value* (added 2026-06-26, B-86)
A holding's value can change two ways, and they are **different actions** the UI must keep distinct:
- **Re-mark to market** = the holding is still yours, its price moved → edit **Current value**. No units leave.
- **Record a sale** = you sold some or all of it → the position shrinks or closes.
  - **Holdings are sellable, not manual-balance-only.** Bonds trade on the secondary market before maturity;
    **American options** can be sold before expiry; crypto/alts sell in part. So Bonds *and* Alternatives both
    expose a **"Record a sale"** action (Stocks/ETFs already sell via lots on the Performance screen).
  - **Partial bond sale** lowers the value **and scales face/par proportionally** (`newFace = face × newValue/value`)
    so coupon income (`face × coupon`) stays correct on the remaining position. **Partial alt sale** just lowers
    value. A **full** sale closes (deletes) the position.
  - **Proceeds are not auto-routed** to a cash account or logged as income yet (decision pending — see
    `finwise-nw-insights-flow.md` §Part 3). Selling only adjusts the holding.
...
5. **One definition per term, surfaced in-context** (Term #11): every user-facing term reads from the single
   `GLOSSARY` (`src/domain/glossary.ts`) via an `<InfoDot>` — so "Bonds", "Alternatives", "Gross income",
   "Contribution room", etc. mean the same thing everywhere. Asset-class group headers (Net Worth, *By class*)
   each carry a dot. New term → add to `GLOSSARY`, never define it ad-hoc on a screen.
6. **Asset-class label is canonical** (`ASSET_CLASS_LABEL` / `assetClassOf`): rows show *what it is* (the class),
   never the `kind` wrapper label (that's what made an alternative read as "Other"). Institution is a separate,
   optional axis (the *By institution* view); a brokerage account also surfaces the tickers it holds.
