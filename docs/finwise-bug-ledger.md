...
| B-44 | LOW | Copy | "Your cash covers about 0.0 months of spending" reads robotically and alarms a user with large liquid investments but $0 in a cash bucket | Home + Net Worth emergency-fund insight copy (cash-only runway) | user-lens TC-03/TC-16 | open | Say "no cash set aside"; acknowledge liquid assets |
| B-45 | LOW | Copy | "86% of your portfolio is a single position" — it's a single account, not a single holding (overclaims concentration) | Insights concentration rule wording | user-lens TC-17 | open | "one account holds 86% — add holdings to see real concentration" |
| B-46 | LOW | Copy | Tips screen "This month at a glance: Income $0.00" contradicts a declared-$276k user; also uses cents against the app-wide whole-dollar style | TipsScreen month-at-a-glance reads logged transactions only | user-lens TC-33 | open | Add "tracks what you log" hint; drop cents |
| B-47 | LOW | Copy | Student's $400/mo parental support labeled "Child support / alimony" in the income manager | income source label mapping for `supportMonthly` | persona walk TC-S4 | open | Relabel parental/family support |
| B-48 | LOW | Copy | "Room left in your 401(k)" nudge shown to users with no earned income / no 401(k) (retiree, gig worker) | Insights 401(k)-room rule not gated on earned income/active 401(k) | persona walk TC-R1/TC-G1 | open | Gate the nudge on earned income + an active 401(k) |
