...
export default function RetirementScreen() {
  ...
  const { investments, savings, addInvestment } = useStore();
  ...
  const monthlyNeeded = monthsToRetire > 0
    ? Math.max(0, (retirementTarget - currentNestEgg) / monthsToRetire)
    : 0;

  // Future value with compound interest
  const r = (parseFloat(expectedReturn) || 7) / 100 / 12;
  const contrib = parseFloat(monthlyContrib) || 0;
  const futureValue = monthsToRetire > 0
    ? currentNestEgg * Math.pow(1 + r, monthsToRetire) +
      contrib * ((Math.pow(1 + r, monthsToRetire) - 1) / r)
    : currentNestEgg;
  ...
  return (
    <ScrollView style={styles.root} contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
      ...
      {/* Contribution source selector shown above modal */}
      {modalVisible && (
        <View style={{ position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(0,0,0,0.4)', zIndex: 999, justifyContent: 'center', padding: 24 }}>
          <View style={{ backgroundColor: Colors.cardBg, borderRadius: 20, padding: 24 }}>
            <Text style={{ fontSize: 18, fontWeight: '700', color: Colors.textPrimary, marginBottom: 4, textAlign: 'center' }}>Log a contribution</Text>
            <Text style={{ fontSize: 13, color: Colors.textSecondary, marginBottom: 16, textAlign: 'center' }}>Which account is this going into?</Text>
            <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 8, marginBottom: 16 }}>
              {CONTRIB_SOURCES.map(src => (
                <TouchableOpacity key={src}
                  style={{ paddingVertical: 8, paddingHorizontal: 14, borderRadius: 20, backgroundColor: contribSource === src ? Colors.primaryLight : Colors.bgSecondary, borderWidth: 0.5, borderColor: contribSource === src ? Colors.primaryMid : Colors.border }}
                  onPress={() => setContribSource(src)}>
                  <Text style={{ fontSize: 13, fontWeight: '600', color: contribSource === src ? Colors.primaryDeep : Colors.textSecondary }}>{src}</Text>
                </TouchableOpacity>
              ))}
            </View>
            <Text style={{ fontSize: 15, fontWeight: '500', color: Colors.textPrimary, marginBottom: 8 }}>Amount ($)</Text>
      ...
      <InputModal
        ...
        message="How much are you contributing today?"
        placeholder="500.00"
        keyboardType="decimal-pad"
        confirmLabel="Add"
        onConfirm={handleAddContrib}
      ...
      <Card>
        ...
        <Text style={styles.inputLabel}>Monthly retirement contribution ($)</Text>
        <Text style={{ fontSize: 11, color: Colors.textTertiary, marginBottom: 6, lineHeight: 16 }}>
          How much do you want to add to your retirement account each month? This is in addition to any savings goals.
        </Text>
        <View style={{ flexDirection: 'row', gap: 8, marginBottom: 8 }}>
          {['100', '250', '500', '750', '1000'].map(amt => (
            <TouchableOpacity
              key={amt}
              style={{
                paddingVertical: 8, paddingHorizontal: 10, borderRadius: 20,
                backgroundColor: monthlyContrib === amt ? Colors.primaryLight : Colors.bgSecondary,
                borderWidth: 0.5,
                borderColor: monthlyContrib === amt ? Colors.primaryMid : Colors.border,
              }}
              onPress={() => setMonthlyContrib(amt)}
            >
              <Text style={{ fontSize: 13, color: monthlyContrib === amt ? Colors.primaryDeep : Colors.textSecondary, fontWeight: '600' }}>
                ${amt}
              </Text>
            </TouchableOpacity>
          ))}
        ...
        <View style={{ backgroundColor: Colors.primaryLight, borderRadius: 8, padding: 10, marginTop: 6 }}>
          <Text style={{ fontSize: 12, color: Colors.primaryDeep, lineHeight: 18 }}>
            💡 Financial advisors recommend saving 10-15% of your income for retirement.
            {monthlyIncome ? ` At your income, that's $${((parseFloat(monthlyIncome) || 0) * 0.12).toFixed(0)}/month.` : ''}
          </Text>
        </View>
        ...
        <View style={styles.returnRow}>
          {['4', '5', '6', '7', '8', '10'].map(r => (
            <TouchableOpacity key={r} style={[styles.returnBtn, expectedReturn === r && styles.returnBtnOn]} onPress={() => setExpectedReturn(r)}>
              <Text style={[styles.returnText, expectedReturn === r && styles.returnTextOn]}>{r}%</Text>
            </TouchableOpacity>
          ))}
        </View>

        <Text style={styles.inputLabel}>Nest egg size</Text>
      ...
      {/* Balance check - income vs expenses + savings + retirement */}
      {monthlyIncome && monthlyContrib && (
        (() => {
          const income = parseFloat(monthlyIncome) || 0;
          const contrib = parseFloat(monthlyContrib) || 0;
          const isNeg = income > 0 && contrib > income * 0.5;
          return isNeg ? (
            <View style={{ backgroundColor: '#FCEBEB', borderRadius: 12, padding: 14, borderWidth: 0.5, borderColor: '#F7C1C1' }}>
              <Text style={{ fontSize: 15, fontWeight: '700', color: '#A32D2D', marginBottom: 4 }}>⚠️ High retirement allocation</Text>
              <Text style={{ fontSize: 13, color: '#791F1F', lineHeight: 20 }}>
                Your retirement contribution (${contrib.toFixed(0)}/month) is over 50% of your income (${income.toFixed(0)}/month). 
                Make sure this leaves enough for expenses and other savings goals.
              </Text>
            </View>
          ) : null;
        })()
      ...
      {monthlyIncome && currentAge && (
        <Card>
          ...
          <View style={styles.projGrid}>
            ...
            <View style={styles.projItem}>
              <Text style={styles.projLabel}>Projected value</Text>
              <Text style={[styles.projValue, { color: onTrack ? Colors.primary : Colors.red }]}>
                ${Math.round(futureValue).toLocaleString()}
              </Text>
            </View>
            <View style={styles.projItem}>
              <Text style={styles.projLabel}>Monthly needed</Text>
              <Text style={styles.projValue}>${Math.round(monthlyNeeded).toLocaleString()}</Text>
            </View>
            <View style={styles.projItem}>
              <Text style={styles.projLabel}>Current savings</Text>
              <Text style={styles.projValue}>${currentNestEgg.toLocaleString()}</Text>
            </View>
          </View>
      ...
      <TipCard color={onTrack ? 'green' : 'amber'}>
        ...
        <Text style={{ fontSize: Typography.sizes.sm, color: onTrack ? Colors.primaryDeep : '#633806', lineHeight: 20 }}>
          {onTrack
            ? `At $${contrib.toFixed(0)}/month at ${expectedReturn}% return, you\'ll have $${Math.round(futureValue).toLocaleString()} by retirement. Stay consistent!`
            : `Try increasing your monthly contribution by $${Math.max(50, Math.round((monthlyNeeded - contrib) / 50) * 50).toLocaleString()}. Even small increases compound significantly over time.`}
        </Text>
      </TipCard>
...
const styles = StyleSheet.create({
  ...
  input: {
    backgroundColor: Colors.bgSecondary, borderRadius: Radii.md, borderWidth: 0.5, borderColor: Colors.border,
    paddingHorizontal: Spacing.md, paddingVertical: 13, fontSize: Typography.sizes.md, color: Colors.textPrimary,
  },
